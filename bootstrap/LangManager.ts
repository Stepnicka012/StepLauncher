...
export type LangData = { [key: string]: string | LangData; };
...
class LangManager {
    ...
    private constructor(langPath?: string) {
        this.langPath = langPath || join(__dirname, '../bootstrap-renderer/Locales');
    ...
    public static init(langPath?: string): LangManager {
        if (!LangManager.instance) {
            LangManager.instance = new LangManager(langPath);
        }
    ...
    public static getInstance(): LangManager {
        if (!LangManager.instance) {
            throw new Error('LangManager no inicializado. Llama a init() primero.');
        }
        return LangManager.instance;
    }
    ...
    private detectSystemLang(): void {
        ...
        try {
            if (typeof ipcRenderer !== 'undefined') {
                sysLang = ipcRenderer.sendSync('get-system-locale');
            }

            if (!sysLang && typeof process !== 'undefined' && process.env) {
                sysLang = process.env.LANG || process.env.LC_ALL || process.env.LC_MESSAGES;
            }

            if (!sysLang && typeof navigator !== 'undefined') {
                sysLang = navigator.language || (navigator as any).userLanguage;
            }

            if (sysLang) {
                this.currentLang = sysLang.split(/[-_]/)[0] || this.fallbackLang;
            } else {
...
export function getLangText(key: string): string {
    return LangManager.getInstance().getText(key);
}
