...
type ConfigValue = string | number | boolean | object | Array<any> | null | undefined;
...
class ConfigManager {
    ...
    private createConfigFile(config: ConfigObject): void {
        try {
            ...
            console.error('Error creando archivo de configuración:', error);
        }
    }

    private saveConfig(config: ConfigObject): void {
        if (this.options.readOnly) {
            console.warn('ConfigManager está en modo solo lectura. No se guardarán cambios.');
            return;
        }
