...
export class NotificationManager {
    ...
    private playSound(path: string): void {
        try {
            ...
            window.StepLauncherLogger.error("Error reproduciendo sonido:", err);
        }
    }

    private playLoop(path: string): HTMLAudioElement {
        const audio = new Audio(path);
        audio.loop = true;
        audio.volume = 1.0;
        audio.play().catch(() => {});
    ...
    public activate(options: NotificationOptions): string {
        ...
        if (options.sound) { this.playSound(options.sound); }
    ...
    private createNotificationElement(id: string, options: NotificationOptions): HTMLElement {
        ...
        closeIcon.src = './Static/Resources/Svg/close.svg';
